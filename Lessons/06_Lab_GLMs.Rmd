...
pm2.5.from.ozone <- lm(data = EPAair, Ozone ~ PM2.5)
summary(pm2.5.from.ozone)
...
library(corrplot)
EPAair.corrplot <- EPAair$PM2.5 %>%
  select(Date:PM2.5) %>%
  na.omit()
summary(EPAair.corrplot)

EPAair.corrplot.plot <- cor(EPAair.corrplot)
corrplot(EPAair.corrplot.plot, method = "ellipse")
corrplot.mixed(EPAair.corrplot.plot, upper = "ellipse")
