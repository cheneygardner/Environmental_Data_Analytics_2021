...
BRA_complete_rain_ts <- ts(BRA_complete$Rainfall, start = c(f_year, f_month), frequency = 12)

BRA_complete_temp_ts <- ts(BRA_complete$Temperature_C, start = c(f_year, f_month), frequency = 12)

plot(BRA_complete_rain_ts)
...
BRA_rain_decomp <- stl(BRA_complete_rain_ts, s.window = "periodic")
plot(BRA_rain_decomp)

BRA_temp_decomp <- stl(BRA_complete_temp_ts, s.window = "periodic")
...
BRA_rain_components <- mutate(BRA_rain_components, observed = BRA_complete$Rainfall, Date = BRA_complete$Date)

BRA_temp_components <- as.data.frame(BRA_rain_decomp$time.series[,1:3])

BRA_temp_components <- mutate(BRA_temp_components, Observed = BRA_complete$Temperature_C, 
                              Date= BRA_complete$Date)


# Exercise 10: Visualize how the trend maps onto the data for both series

ggplot(BRA_rain_components) + geom_line(aes(y= Observed, x = Date), size = 0.25 + geom_line(aes(y=trend, x=Date), color = "#c13d75ff") + geom_hline(yintercept = 0, lty =2) + ylab(expression("Rainfall")))

ggplot(BRA_rain_components) + geom_line(aes(y= Observed, x = Date), size = 0.25 + geom_line(aes(y=trend, x=Date), color = "#c13d75ff") + ylab(expression("Rainfall")))
...
BRA_complete_rain_trend1 <- Kendall::SeasonalMannKendall(BRA_complete_rain_ts)

BRA_complete_rain_trend1
summary(BRA_complete_rain_trend1)

BRA_complete_rain_trend2 <- trend::smk.test(BRA_complete_rain_ts)
BRA_complete_rain_trend2
summary(BRA_complete_rain_trend2)

BRA_complete_temp_trend1 <- Kendall::SeasonalMannKendall(BRA_complete_temp_ts)

BRA_complete_temp_trend1
summary(BRA_complete_temp_trend1)

BRA_complete_temp_trend2 <- trend::smk.test(BRA_complete_temp_ts)
BRA_complete_temp_trend2
summary(BRA_complete_temp_trend2)


#tau strength of trend

# Exercise 12: Now remove the seasonality and apply another test. Compare the results with what you obtained on Exercise 9.

MannKendall()
BRA_complete_rain_trend_combo <- Kendall::MannKendall(BRA_complete_rain_ts-BRA_rain_components$seasonal)
BRA_complete_rain_trend_combo
summary(BRA_complete_rain_trend_combo)



BRA_complete_temp_trend_combo <- Kendall::MannKendall(BRA_complete_temp_ts)

#originalseries - #seasonal component
